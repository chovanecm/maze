...
cdef class Result:
        ...
        cdef public numpy.int64_t[:] goal
        cdef public numpy.int64_t[:,:] directions
        def __init__(self,numpy.ndarray[numpy.int64_t, ndim=2] distances,
                     numpy.ndarray[numpy.int64_t, ndim=2] maze,
                     numpy.ndarray[numpy.int64_t, ndim=3] predecessors,
                      numpy.ndarray[numpy.int64_t, ndim=1] goal):
        ...
        def path(self, row, column):
            ...
            t_goal = tuple(self.goal)
            shortest_path = []
            node = (row, column)
            shortest_path.append(node)
            while node != t_goal:
                node = tuple(self.predecessors[node])
                shortest_path.append(node)
...
def analyze(maze):
    ...
    return Result(distances, maze, predecessors, goal)


def dijkstra(maze, start):
    INF = maze.shape[0] * maze.shape[1] * 2
    distances = np.full(shape=maze.shape, fill_value=INF, dtype=np.int64)
    distances[tuple(start)] = 0
    predecessors = np.full(shape=(maze.shape[0], maze.shape[1], 2),
                           fill_value = (-100, -100), dtype=(np.int64,
                                                             np.int64))
    q = PriorityQueue()
...
def get_maze_from_file(file):
    maze = np.fromfile(file, dtype=np.int64)
    #assume square shape
    dim = sqrt()
    return maze.reshape(())
